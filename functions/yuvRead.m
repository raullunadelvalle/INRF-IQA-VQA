...
%           width 	:   Frame width
%           hight 	:   Frame hight
%           nFrame 	:   Vector of frame numbers to be read
% 
% Outputs:
...
fclose(fid); % Raul: esto lo añado yo 
length = 1.5 * width * height;  % Length of a single frame

y = uint8(zeros(height,   width,   size(nFrame,2)));
u = uint8(zeros(height/2, width/2, size(nFrame,2)));
v = uint8(zeros(height/2, width/2, size(nFrame,2)));
